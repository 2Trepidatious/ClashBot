...
def load_war_stars():
    """Load war stars data from a JSON file."""
    if os.path.exists(WAR_STARS_FILE):
        with open(WAR_STARS_FILE, "r") as file:
            return json.load(file)
    return {}

def save_war_stars(data):
    """Save war stars data to a JSON file."""
    with open(WAR_STARS_FILE, "w") as file:
...
def update_war_stars(attacks, war_stars_data, tracked_attacks):
    """Update the war stars data with the results from the current war."""
    for attack in attacks:
        attack_key = (attack["attacker"], attack["defender"])
        if attack_key not in tracked_attacks:
            attacker = attack["attacker"]
            stars = attack["stars"]
            if attacker not in war_stars_data:
                war_stars_data[attacker] = 0
            war_stars_data[attacker] += stars
            tracked_attacks.append(attack_key)
    return war_stars_data, tracked_attacks
...
async def check_war_updates():
    ...
    # Load existing war stars and tracked attacks data
    war_stars_data = load_war_stars()
    ...
    # Update and save war stars data
    war_stars_data, tracked_attacks = update_war_stars(attacks, war_stars_data, tracked_attacks)
    save_war_stars(war_stars_data)
